...
function memorize(){
    ...
    if (count<2){
        ...
        else if (chose.dataset.name === firstCard) {
            firstCard=""
        ...
        if (firstCard !='' && secondCard !=''){
            if (firstCard === secondCard + 'c' || secondCard === firstCard + 'c') {

                setTimeout(removeBounary,500)    
...
function removeBounary() {
    var chosen = document.querySelectorAll('.chosen');
    chosen.forEach(card => {
        card.classList.remove('chosen'); //boundary is removed
